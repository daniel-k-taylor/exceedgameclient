...
var default_deck = CardDefinitions.get_deck_from_str_id("jacko")
...
func test_jacko_countdown():
	...
	assert_true(game_logic.do_boost(player1, TestCardId3))
	advance_turn(player2)
	give_player_specific_card(player1, "jacko_foreverelysiondriver", TestCardId4)
	assert_true(game_logic.do_boost(player1, TestCardId4))
	advance_turn(player2)
	give_player_specific_card(player1, "gg_normal_sweep", TestCardId5)
	assert_true(game_logic.do_boost(player1, TestCardId5))
	advance_turn(player2)
	execute_strike(player1, player2, "jacko_countdown", "gg_normal_sweep", [0], [])
	assert_eq(player1.gauge.size(), 2)
	assert_eq(player1.continuous_boosts.size(), 0)
	validate_positions(player1, 3, player2, 6)
...
func test_jacko_cheerservant():
	...
	execute_strike(player1, player2, "jacko_cheerservanton", "gg_normal_sweep", [], [])
	assert_true(game_logic.do_choose_from_boosts(player1, [TestCardId4]))
	assert_true(game_logic.do_choice(player1, 0))
	assert_true(game_logic.do_choice(player1, 0))
	assert_eq(player1.gauge.size(), 2)
	assert_eq(player1.continuous_boosts.size(), 1)
	assert_eq(player1.continuous_boosts[0].id, TestCardId4)
