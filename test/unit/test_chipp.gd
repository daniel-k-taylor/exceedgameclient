...
var default_deck = CardDataManager.get_deck_from_str_id("chipp")
...
func test_chipp_alpha_boost():
	position_players(player1, 3, player2, 5)
	give_player_specific_card(player1, "chipp_alphablade", TestCardId3)
	assert_true(game_logic.do_boost(player1, TestCardId3))
	advance_turn(player2)
	execute_strike(player1, player2, "gg_normal_slash", "gg_normal_focus", [0], [1])
	validate_positions(player1, 6, player2, 4)
	validate_life(player1, 23, player2, 27)

func test_chipp_resshou_strike():
	position_players(player1, 3, player2, 5)
	assert_eq(player1.hand.size(), 5)
	execute_strike(player1, player2, "chipp_resshou", "gg_normal_slash", [1, 1], [])
	validate_positions(player1, 2, player2, 5)
	validate_life(player1, 30, player2, 28)
	assert_eq(player1.hand.size(), 8)

func test_chipp_gammablade_boost():
	position_players(player1, 3, player2, 5)
	give_player_specific_card(player1, "chipp_gammablade", TestCardId3)
	assert_true(game_logic.do_boost(player1, TestCardId3))
	validate_positions(player1, 1, player2, 5)
	advance_turn(player2)
	execute_strike(player1, player2, "gg_normal_focus", "gg_normal_grasp", [], [0])
	assert_true(game_logic.do_choice(player1, 1))
	validate_positions(player1, 6, player2, 4)
	validate_life(player1, 30, player2, 26)

func test_chipp_genrouzan_boost():
	position_players(player1, 4, player2, 5)
	give_player_specific_card(player1, "chipp_genrouzan", TestCardId3)
	assert_true(game_logic.do_boost(player1, TestCardId3))
	execute_strike(player2, player1, "gg_normal_grasp", "gg_normal_grasp", [0], [0, 0])
	validate_positions(player1, 3, player2, 2)
	validate_life(player1, 30, player2, 27)

func test_chipp_genrouzan_boost_goesaway():
	position_players(player1, 4, player2, 5)
	give_player_specific_card(player1, "chipp_genrouzan", TestCardId3)
	assert_true(game_logic.do_boost(player1, TestCardId3))
	advance_turn(player2)
	assert_eq(player1.hand.size(), 6)
	execute_strike(player1, player2, "gg_normal_grasp", "gg_normal_sweep", [0, 0], [1])
