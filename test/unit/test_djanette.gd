...
func test_djanette_blackdeath_transform_diabolic_strike():
	...
	position_players(player1, 3, player2, 7)
	add_transform(player1, "djanette_blackdeath")
	var diabolic_id = give_player_specific_card(player1, "standard_normal_grasp")
	assert_true(game_logic.do_character_action(player1, [], 0))
	var cards_in_hand = []
	for card in player1.hand:
		cards_in_hand.append(card.id)
	assert_eq(cards_in_hand.size(), 6)
	assert_true(game_logic.do_relocate_card_from_hand(player1, [diabolic_id]))
	assert_eq(player1.hand.size(), 6)
	assert_eq(player1.hand[0].id, cards_in_hand[0])
	assert_eq(player1.hand[1].id, cards_in_hand[1])
	assert_eq(player1.hand[2].id, cards_in_hand[2])
	assert_eq(player1.hand[3].id, cards_in_hand[3])
	assert_eq(player1.hand[4].id, cards_in_hand[4])
	assert_eq(player1.set_aside_cards.size(), 1)
	assert_eq(player1.set_aside_cards[0].id, diabolic_id)
	assert_true(game_logic.do_force_for_effect(player1, [player1.hand[0].id], true))
	assert_eq(player1.set_aside_cards.size(), 1)
	assert_eq(player1.set_aside_cards[0].id, diabolic_id)
	execute_strike(player1, player2, "standard_normal_dive", "standard_normal_sweep", false, false,
