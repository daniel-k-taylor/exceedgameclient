...
var default_deck = CardDefinitions.get_deck_from_str_id("guile")
...
func give_gauge(player, amount, card_type : String = ""):
	for i in range(amount):
		for deck_index in range(player.deck.size()):
			if card_type and player1.deck[deck_index].definition['type'] == card_type:
				player.add_to_gauge(player.deck[deck_index])
				player.deck.remove_at(deck_index)
				break
			elif not card_type:
				player.add_to_gauge(player.deck[deck_index])
				player.deck.remove_at(deck_index)
...
func test_guile_cc_gauge_ultra():
	position_players(player1, 3, player2, 6)
...
func test_guile_exceed_cc_gauge_strike():
	position_players(player1, 3, player2, 6)
	player1.exceed()
	give_gauge(player1, 1, "normal")
	give_player_specific_card(player1, "standard_normal_assault", TestCardId3)
