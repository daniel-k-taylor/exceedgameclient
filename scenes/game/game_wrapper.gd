...
func submit_finish_reshuffle(player : Enums.PlayerId, manual : bool, followup_effect) -> bool:
	var game_player = _get_player(player)
	return current_game.do_finish_reshuffle(game_player, manual, followup_effect)
